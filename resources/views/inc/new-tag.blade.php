<section class="new-post flex justify-center items-center flex-col px-4 md:px-0">
    ...
    <section class="new-post-image my-5"
        style="background-image: url({{ asset('storage/' . $tagPost->thumbnail ) }});background-position: center center;background-size: cover;">
    </section>
    <p class="tracking-wider leading-8 text-base">{{ Str::limit($tagPost->body, 220) }}</p>
    <a href="{{ route('single-post', $tagPost->id) }}" class="pb-2 font-semibold border-b my-4 hover:text-teal-600">Continue Reading</a>
